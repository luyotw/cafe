...
CAFE is designed for **individual developers** who wish to leverage CLI agent tools to accelerate their development workflow in a more efficient and intelligent way. If you are looking to:

*   **Utilize CLI agent tools more efficiently:** Automate tedious development processes such as writing specifications, development, code review, and generating pull requests, allowing you to focus more on core creative work.
*   **Achieve more controllable AI-generated code:** CAFE's multi-stage workflow breaks down complex tasks, guiding AI to focus on a single objective at each step, improving the stability and controllability of its output.
*   **Improve the maintainability of AI-generated code:** Through stages like code review and implementation planning, CAFE ensures that AI-generated code is not only functional but also possesses good structure and readability, reducing the time and effort required for future refactoring.

> To achieve high-quality code, there's no magic bullet. Users still need to dedicate time to review AI-generated content. CAFE's purpose is to minimize the user's review time to the greatest extent possible, not to eliminate review entirely.
...
Integrates with various CLI agents (e.g., Claude Code, Cursor CLI), enabling flexible assignment of agents and models for each role to optimize both cost and performance.
