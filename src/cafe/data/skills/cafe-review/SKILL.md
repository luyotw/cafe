...
- `## Todo List` 的每個 open blocker 必須使用 canonical 單行格式 ``- [ ] `BLK-NNN` — Source: `review` — Work: ... — Closure: ... — Evidence: ...``；沒有 open blocker 時區段必須只寫 canonical marker `No actionable work.`，不得留空；區段外 checkbox 與 Follow-up Proposal 不屬於 correction work。
