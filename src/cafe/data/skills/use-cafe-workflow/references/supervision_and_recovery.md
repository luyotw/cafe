...
- the same failure has not returned after one user-authorized safe retry;
...
| 3. A mandatory, `user_required`, permission, capability, strategy, scope, external-effect, model-chain, or other user-owned decision is pending | Present it to the user through its declared boundary. Do not ask the phase agent to infer or approve it. |
...
| 6. The same phase-agent failure returns after one user-authorized retry | Keep the recovery task user-owned. Consult the responsible phase agent once with `cafe chat <role> --phase <step> -p "<bounded diagnostic question>"`, then inspect again before recommending another recovery action. |
| 7. Visible phase progress exists but there is no valid handoff, or reported success conflicts with current state | Consult the responsible phase agent with the same bounded chat form to identify completed work, the missing boundary, and one bounded next action. Do not reconstruct the handoff yourself. |
| 8. One isolated, clearly transient phase-agent failure has an unchanged safe idempotent retry option | Present every declared recovery option and practical consequence, and recommend at most one bounded retry under the unchanged contract. Do not submit the choice for the user. |
...
- `cafe chat` cannot complete a HumanTask, edit an artifact, grant authority,
  deliver input to another iteration, or prove completion. Apply a diagnostic
  conclusion only through an existing legal task, input, correction, or
  authorization path; otherwise retain the pause.
