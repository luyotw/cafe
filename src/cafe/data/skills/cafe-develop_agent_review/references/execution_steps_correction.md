...
[ ] Resolve every blocking finding, rerun targeted checks, and rerun both reviewers concurrently after each correction
...
- playbook 的下一個 step（預設為 `pr`）：All issues fixed and both subagents report no blocking issues in the current review round
