...
- 讀 `mandate.out_of_mandate`（清單）：
  - 若本 issue 的意圖與清單上任一項重疊（例如 deployment configuration、payment integration、或 user 額外指定的項目），**不要直接寫進 spec**。改為輸出 `questions.xml` 並依 workflow-common 暫停給 `user`，請其確認是否例外納入或排除。
