...
from ._lifecycle import activate, adopt_legacy, evaluate, replace
...
def evaluate_driver_entry(command: DriverEntryRequest) -> DriverEntryResult:
    ...
    if contract["driver"]["mode"] == "event-driven":
        event = MappingProxyType({"clis": tuple(dict(item) for item in contract["driver"]["clis"])})
    return DriverEntryResult(
        freshness=freshness,
        revision=contract["revision"]["generation"],
        contract_sha256=digest,
        runtime=MappingProxyType({"driver": MappingProxyType(dict(contract["driver"])), "checkout": MappingProxyType(dict(contract["checkout"]))}),
        event=event,
        proactive_review=tuple(MappingProxyType(dict(item)) for item in contract["proactive_review"]["phase_decisions"]),
        generic_inputs=MappingProxyType(generic),
    )
