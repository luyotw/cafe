...
from cafe.core.driver_policy import POLICY_KEYS, DriverPolicyContract, policy_dict
    ...
    def ensure_prepare_target_available(issue_dir: Path) -> None:
        if (Path(issue_dir) / "blackboard.json").exists():
            raise PrepareWouldClobberError(
                "active workflow state exists; use cafe update-driver-policy with complete v2 choices"
            )
