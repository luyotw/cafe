"""測試 Phase base class  user_input 處理功能."""
...
class TestUpdateIterationHistory:
    ...
    def test_update_existing_file(self, tmp_path: Path) -> None:
        ...
        # 應該同時包含 user_input and response
        assert data["user_input"] == user_input
...
class TestTwoPhaseWorkflow:
    ...
    def test_complete_iteration_workflow(self, tmp_path: Path) -> None:
        ...
        # Round 1：先儲存 user_input
        phase.iteration = 1
        phase._save_user_input("Round 1使用者輸入")

        # 模擬 agent 執行後更新
        phase._update_iteration_history(
            phase_specific_data={"response": "Round 1 Agent 回應"},
        ...
        # Round 2：使用者提供修改意見
        phase.iteration = 2
        phase._save_user_input("Round 2使用者修改意見")

        # Agent 再次執行
        phase._update_iteration_history(
            phase_specific_data={"response": "Round 2 Agent 回應"},
        ...
        assert data1["user_input"] == "Round 1使用者輸入"
        assert data1["response"] == "Round 1 Agent 回應"
        assert data1["status_code"] == "CAFE_NEED_CLARIFICATION"

        # Round 2
        assert data2["iteration"] == 2
        assert data2["user_input"] == "Round 2使用者修改意見"
        assert data2["response"] == "Round 2 Agent 回應"
    ...
    def test_user_input_preserved_on_agent_failure(self, tmp_path: Path) -> None:
        ...
        # 不應該有 response or status_code（因為沒有呼叫 _update_iteration_history）
