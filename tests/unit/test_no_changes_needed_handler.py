...
class _FakeDevelopPhase:
    def __init__(self, issue_dir: Path, iteration: int = 2) -> None:
...
def test_no_changes_handler_pauses_when_reasoning_present(tmp_path: Path, monkeypatch) -> None:
    ...
    iteration_dir.mkdir(parents=True, exist_ok=True)
    output_file = iteration_dir / "output.md"
    output_file.write_text("Reviewer feedback is unnecessary because tests already cover it.", encoding="utf-8")
...
def test_user_input_collector_confirm_routes_manual_handoff(mock_prompt_list, tmp_path: Path) -> None:
    ...
    store.record_event(state, "step_completed", {"step": "develop", "status_code": "no_changes_needed"})

    phase = _FakeDevelopPhase(issue_dir, iteration=2)
...
def test_user_input_collector_chat_then_confirm(mock_prompt_list, mock_chat, tmp_path: Path) -> None:
    from cafe.core.blackboard import BlackboardStore
