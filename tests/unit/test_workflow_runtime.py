...
def test_local_review_fix_now_delivers_pr_curation_before_develop(
    tmp_path: Path,
) -> None:
    """A PR self-continuation keeps its feedback target across correction routing."""
    from cafe.core.todo import workflow_feedback_todo_items
    from cafe.core.workflow_feedback import WorkflowFeedbackLedger
    from cafe.ui.human_tasks import apply_human_task_payload

    issue_dir = tmp_path / ".cafe" / "issues" / "local-review-pr-correction"
    _write_publication_contract(
        issue_dir, playbook_id="direct-subagent-review", persisted=False
    )
    playbook = PlaybookLoader().load("direct-subagent-review")
    store = BlackboardStore(issue_dir)
    state = store.load_or_create("pr", playbook_id="direct-subagent-review")
    policy, binding = resolve_step_human_task(
        playbook_data=playbook,
        step_name="pr",
        trigger="confirm_output",
    ...
    applied = apply_human_task_payload(
        ...
        raw_payload={
            "task": policy.id,
            "decision": "fix_now",
            "feedback": "Fix the locally reviewed regression.",
            "human_task_id": task.id,
        },
        source="test",
    )

    assert applied.target == "pr"
    ledger = WorkflowFeedbackLedger(issue_dir)
    feedback = ledger.pending(target_step="pr")
    assert len(feedback) == 1
    output = issue_dir / "pr" / "iteration_002" / "output.md"
    output.parent.mkdir(parents=True)
    digest = sha256(feedback[0].source_identity.encode("utf-8")).hexdigest()
    item_id = f"WF-{digest[:12].upper()}"
    output.write_text(
        "## Todo List\n"
        f"- [ ] `{item_id}` — Source: `workflow_feedback` — "
        "Work: Fix the locally reviewed regression. — "
    ...
    def executor(step_name: str, _step: dict, _state: object) -> StepExecutionResult:
        ...
        return StepExecutionResult(
            response="curated local review feedback",
            artifacts={"pr_result": str(output)},
            agent_invoked=True,
            feedback_source_identities=(feedback[0].source_identity,),
        )
    ...
    assert result.final_status_code == "BATON_MANUAL_HANDOFF", result.detail
    assert ledger.pending(target_step="pr") == []
    current = store.load_or_create("pr")
    assert current.current_step == "develop"
    assert current.handoff_contract is not None
    assert current.handoff_contract.from_step == "pr"
    assert current.handoff_contract.to_step == "develop"
    delivered = workflow_feedback_todo_items(
        ledger.path,
        target_step=current.current_step,
        source_by_kind={"local_review": "workflow_feedback"},
        id_prefix_by_kind={"local_review": "WF"},
        source_identities=(feedback[0].source_identity,),
    )
    assert [item.work for item in delivered] == ["Fix the locally reviewed regression."]
    assert not any(
        event.data.get("status_code") == "INVALID_FEEDBACK_DELIVERY"
