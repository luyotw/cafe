...
from cafe.core.hooks.native import InitialInputProviderResolver
...
@pytest.mark.parametrize("paused_for_user", [False, True])
def test_declared_removed_step_migration_resumes_at_replacement_and_cancels_tasks(
    tmp_path: Path, paused_for_user: bool
) -> None:
    issue_dir = tmp_path / ".cafe" / "issues" / "migrated-direct-qa"
    playbook = {
        "playbook": {"id": "direct-qa"},
        "migrations": {
            "step_redirects": {"spec": "develop"},
            "artifact_aliases": {"spec": "requirements"},
        },
        "entry_point": "develop",
        "steps": {
            "develop": {
    ...
    state = store.load_or_create("spec", playbook_id="direct-qa")
    legacy_spec = issue_dir / "spec" / "iteration-1" / "output.md"
    legacy_spec.parent.mkdir(parents=True, exist_ok=True)
    legacy_spec.write_text("# Existing requirements\n", encoding="utf-8")
    store.set_artifact(state, "spec", str(legacy_spec))
    ...
    if paused_for_user:
        ...
        task = records.materialize(
            ...
            prompt="Review the old specification.",
            expected_result={"input_schema": "decision"},
            continuations={"confirm": "develop", "revise": "spec"},
            assignee_type="human",
        )

    calls: list[str] = []

    def executor(step_name: str, step: dict, runtime_state: object) -> StepExecutionResult:
        calls.append(step_name)
        resolved = InitialInputProviderResolver().run(
            stage="prepare_input",
            phase=SimpleNamespace(issue_dir=issue_dir, iteration=1),
            step_name=step_name,
            step_def=step,
            output_file=issue_dir / "develop" / "iteration_001" / "output.md",
            blackboard_state=runtime_state,
            context={},
        )
        assert resolved.context_updates == {"user_input": "# Existing requirements"}
    ...
    migrated = store.load_or_create("develop", playbook_id="direct-qa")
    assert migrated.current_step == "done"
    assert migrated.artifacts["spec"].path == str(legacy_spec)
    assert migrated.artifacts["requirements"].path == str(legacy_spec)
    assert legacy_spec.read_text(encoding="utf-8") == "# Existing requirements\n"
    events = [event for event in migrated.events if event.event_type == "workflow_step_migrated"]
    assert events[-1].data["from_step"] == "spec"
    assert events[-1].data["to_step"] == "develop"
    if task is not None:
        assert records.get_task(task.id).status is HumanTaskStatus.CANCELLED


@pytest.mark.parametrize(
    ("owner", "method_name", "fail_on_call"),
    [
        (BlackboardStore, "put_artifact", 1),
        (BlackboardStore, "record_event", 1),
        (HumanTaskRecordStore, "cancel", 1),
        (BlackboardStore, "update_handoff_contract", 1),
        (BlackboardStore, "set_current_step", 1),
        (BlackboardStore, "record_event", 2),
    ],
)
def test_declared_step_migration_recovers_after_every_durable_write(
    tmp_path: Path,
    ...
    issue_dir = tmp_path / method_name
    playbook = {
        "playbook": {"id": "direct-qa"},
        "migrations": {
            "step_redirects": {"spec": "develop"},
    ...
    store = BlackboardStore(issue_dir)
    state = store.load_or_create("spec", playbook_id="direct-qa")
    legacy_spec = issue_dir / "spec.md"
    legacy_spec.write_text("legacy\n", encoding="utf-8")
    store.set_artifact(state, "spec", str(legacy_spec))
    store.update_handoff_contract(
        state,
        from_step="spec",
    ...
    def interrupt_after_write(instance: object, *args: object, **kwargs: object) -> object:
        nonlocal call_count
        result = original(instance, *args, **kwargs)
        call_count += 1
        if call_count == fail_on_call:
            raise RuntimeError(f"interrupted after {method_name}")
        return result

    monkeypatch.setattr(owner, method_name, interrupt_after_write)
    with pytest.raises(RuntimeError, match=method_name):
        runtime._apply_declared_step_migration()
    ...
    resumed._apply_declared_step_migration()

    recovered = BlackboardStore(issue_dir).load_or_create(
        "develop", playbook_id="direct-qa"
    )
    assert recovered.current_step == "develop"
    assert recovered.handoff_contract is not None
    assert recovered.handoff_contract.from_step == "develop"
    assert recovered.handoff_contract.to_step == "develop"
    assert recovered.artifacts["requirements"].path == str(legacy_spec)
    assert HumanTaskRecordStore(issue_dir).get_task(task.id).status is HumanTaskStatus.CANCELLED
    events = [event for event in recovered.events if event.event_type == "workflow_step_migrated"]
    assert len(events) == 1
    assert events[0].data["cancelled_task_ids"] == [task.id]
