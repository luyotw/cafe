"""Integration tests for default-playbook plan clarification (mocked executor)."""
...
def test_plan_need_clarification_pauses_at_user(tmp_path: Path) -> None:
    """Plan need_clarification with auto_continue=False pauses workflow at user step."""
    issue_dir = tmp_path / ".cafe" / "issues" / "issue-plan-clarify-pause"
    ...
    result = runner.run(start_step="plan", max_transitions=10)
...
def test_plan_clarification_then_confirmed_reaches_develop(tmp_path: Path) -> None:
    """Plan need_clarification (auto_continue) then confirmed advances toward develop."""
    issue_dir = tmp_path / ".cafe" / "issues" / "issue-plan-clarify-develop"
    playbook = _load_default_playbook()
    plan_calls = 0
    ...
    develop_started = [
        ...
        if e.event_type == "step_started" and e.data.get("step") == "develop"
    ]
    assert develop_started
