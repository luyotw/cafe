...
[ ] Start `release-check` only when the effective playbook explicitly assigns `release-check` to the current `develop` step or the user explicitly requests it; a declaration assigning it to a separate `verify` or `verification` step does not authorize this `develop` step; risk, scale, precaution, PR preparation, review/proactive review, and agent judgment are not authority
